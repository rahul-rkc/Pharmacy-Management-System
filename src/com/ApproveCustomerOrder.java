	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			if(i==1){
				...
				System.out.println("User balance: "+blnc);
				final_blnc = blnc-bill;
				ps1.setInt(1, final_blnc);
				ps1.setString(2, cust_id);
				ps1.executeUpdate();
				System.out.println("Customer current Balance: "+final_blnc);
				ResultSet rs4 = ps4.executeQuery();
				while(rs4.next()){
					blnc = rs4.getInt("blnc");
				}
				System.out.println("Admin balance: "+blnc);
				final_blnc = blnc+bill;
				ps1.setInt(1, final_blnc);
				ps1.setString(2, admin_id);
				ps1.executeUpdate();
				System.out.println("Vendor current Balance: "+final_blnc);
				}
			}else{
